...
const ChatView: React.FC<{
  ...
  const handleAttachment = async () => {
    ...
    if (!res.canceled) {
      ...
      const tempId = 'temp-' + Date.now();
      const nm = { id: tempId, type: 'image', media_url: uri, sender_id: currentUserId, created_at: new Date().toISOString() };
      setMessages(p => [nm, ...p]);
      try { await sendImageMessage(convId, currentUserId, uri); } catch { setMessages(p => p.filter(m => m.id !== tempId)); Alert.alert('Failed to send', 'Image could not be delivered.'); }
    }
  };

  const handleReactionTap = async (msg: any, emoji: string) => {
    const reactions: any[] = msg.reactions ?? [];
    const existingIdx = reactions.findIndex((r: any) => r.emoji === emoji && r.user_id === currentUserId);
    const isRemoving = existingIdx !== -1;
    setMessages(prev => prev.map(m => {
      if (m.id !== msg.id) return m;
      const updated = isRemoving
        ? (m.reactions ?? []).filter((_: any, i: number) => i !== existingIdx)
        : [...(m.reactions ?? []), { id: 'temp-' + Date.now(), emoji, user_id: currentUserId }];
      return { ...m, reactions: updated };
    }));
    setReactionTarget(null);
    try {
      if (isRemoving) await removeReaction(msg.id, currentUserId, emoji);
      else await addReaction(msg.id, currentUserId, emoji);
    } catch {
      setMessages(prev => prev.map(m => m.id === msg.id ? { ...m, reactions: reactions } : m));
    }
  };
